...
class MessagesManager: ObservableObject {
    ...
    func getMessages() {
        db.collection("messages").addSnapshotListener { querySnapshot, error in
            ...
            guard let documents = querySnapshot?.documents else {
                print("Error fetching documents, Please try again in few moments: \(String(describing: error))")
                return
            }
            ...
            self.messages = documents.compactMap { document -> Message? in
                do {
                    ...
                    print("Error decoding document into Message, Please try again in few moments: \(error)")
    ...
    func sendMessage(text: String) {
        do {
            ...
            // If we run into an error, print the error in the console
            print("Error adding message to Firestore, Please try again in few moments: \(error)")
        }
    }
}
